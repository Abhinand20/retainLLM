...
if [ "$machine_arch" == "armv7l" ] || [ "$machine_arch" == "aarch64" ]; then
    docker run --rm --gpus all --init --ulimit core=0 -p 8080:8070 -p 8081:8071 grobid/grobid:0.8.0-arm
else
    docker run --rm --init --ulimit core=0 -p 8080:8070 -p 8081:8071 lfoppiano/grobid:latest-crf
fi